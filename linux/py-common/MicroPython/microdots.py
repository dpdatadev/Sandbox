...
##################################################################
# End of microdot/lib.py
##################################################################
##################################################################
# MicroDot MULTIPART extension:
import os
...
class FormDataIter:
    ...
    async def _fill_buffer(self):
        ...
        self.buffer += await self.request.stream.read(
            self.buffer_size
            + self.extra_size
            - len(self.buffer)  # pyright: ignore[reportArgumentType]
        )  # type: ignore
    ...
    async def _read_buffer(self, n=-1):
        ...
        while n == -1 or len(data) < n:
            await self._fill_buffer()
            s = self.buffer.split(self.boundary, 1)  # type: ignore
    ...
    async def copy(self, max_memory_size=None):
        ...
        while True:
            tmpname = "".join(
                [
                    choice(
                        "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"  # pyright: ignore[reportArgumentType]
        ...
        self._read = read  # pyright: ignore[reportGeneralTypeIssues]
        self._close = close
        return self
...
if __name__ == "__main__":
    #################################################
    # Start Microdot web server (Replaced PicoWeb)
    ...
    <html>
        ...
        <body>
            <div>
                <h1>Configure Nodes for Monitoring</h1>
                <p>PicoPeek</p>
                <form method="POST" enctype="multipart/form-data" action="/config">
                    <label for="node_name">Node Name:</label><br>
                    <input type="text" id="node_name" name="node_name"><br><br>
                    <label for="ip_address">IP Address:</label><br>
                    <input type="text" id="ip_address" name="ip_address"><br><br>
                    <label for="config_file">Upload Config File:</label><br>
                    <input type="file" id="config_file" name="config_file"><br><br>
    ...
    @app.get("/config")
    async def index(request):
        return form_data_html, 200, {"Content-Type": "text/html"}
